...
from typing import List, Union, Callable, Dict, Iterable, Optional
...
def greedily_divide_into_L_sets(
    ...
    for k in range (0, len(Vs_per_level)):
        ...
        for u in V_kr:
            ...
            setattr(G.V[u], "received_offers_summary_dict", summary)
            setattr(G.V[u], "merge_set", None)
        for u in V_kr:
            for v, count in sorted(G.V[u].received_offers_summary_dict.items(), key=lambda x: -x[1]):
                if not determine_if_should_merge(k, count):
                    break
                if G.V[u].merge_set is None and G.V[v].merge_set is None:
                    G.V[u].merge_set = set([u, v]); G.V[v].merge_set = G.V[u].merge_set
                elif G.V[v].merge_set is None:
                    G.V[u].merge_set.add(v); G.V[v].merge_set = G.V[u].merge_set
                elif G.V[u].merge_set is None: 
                    G.V[v].merge_set.add(u); G.V[u].merge_set = G.V[v].merge_set 
                else:
                    G.V[u].merge_set.add(v)
                    for w in G.V[v].merge_set:
                        G.V[u].merge_set.add(w); G.V[w].merge_set = G.V[u].merge_set
        for u in V_kr:
            ...
            for c in C: 
                ...
                if score < best_score:
                    best_score = score; best_color = c 
                apply_coloring_based_on_map(G, base_coloring)
            if best_color is not None:
                for x in mu:
                    G.V[x].L_set_index = best_color
                apply_best_coloring(G, [best_color], V_mu, EdgeConsideringPolicy.BOTH, V_mu)
...
class LSetCreator:
    ...
    def create_initial_L_sets(self, G: Graph) -> List[List[int]]:
        ...
        for i in range (len(Vs_per_level)):
            cum_V_counts_per_level[i] = len(Vs_per_level[i]) + (cum_V_counts_per_level[i-1] if i > 0 else 0)
