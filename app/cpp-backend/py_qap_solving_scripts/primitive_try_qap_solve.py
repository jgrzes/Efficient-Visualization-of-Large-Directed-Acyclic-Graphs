...
class LinkedListElement:
    def __init__(
        self, i: int, list_ref, contents: Any, previous_element=None, next_element=None
    ):
        self._index = i
...
def primitive_try_qap_solve(
    ...
    for i, j, unused_value in non_zero_F_cells:
        ...
        elif (in_nrll_sets_i and not in_nrll_sets_j) or (
            not in_nrll_sets_i and in_nrll_sets_j
        ):
            if not in_nrll_sets_i:
                i, j = j, i
...
def primitve_try_qap_solve_with_borders_fixed(
    ...
    for i, j, _ in non_zero_F_cells:
        pi = find_parent(i, parent_map=parent)
        pj = find_parent(j, parent_map=parent)
        plb, prb = find_parent(lb, parent_map=parent), find_parent(
            rb, parent_map=parent
        )
        ...
        if pi == plb:
            ...
            if not in_nrll_sets_i and not in_nrll_sets_j:
                ...
                for c, node_c in nrll_i.contents_map.items():
                    ...
                    if F_matrix[j, c] != 0:
                        cum_sum_for_add_as_head += (index_c + 1) * F_matrix[j, c]
                        cum_sum_for_add_as_tail += (nrll_i_size - index_c) * F_matrix[
                            j, c
                        ]
            ...
            # Alternatively: mapped_elements[i].list_ref is mapped_elements[j].list_ref
                ...
                for x, node_x in nrll_j.contents_map.items():
                    ...
                    for c, node_c in nrll_i.contents_map.items():
                        index_c = node_c.index
                        if F_matrix[x, c] != 0:
                            cum_sum_1 += (size_j - index_x + index_c) * int(
                                F_matrix[x, c]
                            )
                            cum_sum_2 += (size_i - index_c + index_x) * int(
                                F_matrix[x, c]
                            )
                ...
                for x, node_x in nrll_j.contents_map.items():
                    ...
                    for c, node_c in nrll_i.contents_map.items():
                        index_c = node_c.index
                        if F_matrix[x, c] != 0:
                            cum_sum_3 += (size_j - index_x + index_c) * int(
                                F_matrix[x, c]
                            )
                            cum_sum_4 += (size_i - index_c + index_x) * int(
                                F_matrix[x, c]
                            )
