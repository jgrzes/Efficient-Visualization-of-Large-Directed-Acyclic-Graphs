...
def write_biq_bin_source_path(
    F_matrix: np.ndarray, D_matrix: PretendMatrix, biq_bin_source_filepath: str
) -> None:
    n = F_matrix.shape[0]
                ...
                for k in range(1, n + 1):
                    for l in range(1, n + 1):
                        if k == l:
                            continue
                        biq_bin_file.write(
                            f"{i*n + k} {j*n + l} {int(F_matrix[i, j] * D_matrix[k, l])}\n"
                        )
...
def fill_colour_remapping_by_performing_qap(
    ...
    max_cut_problem_transformation_filepath: str,
) -> None:
    Q = deque()
    ...
    while len(Q) > 0:
        ...
        if node.parent is None:
            ...
            R, successful = primitive_try_qap_solve(
            ...
            if not successful:
                write_biq_bin_source_path(
                    F_for_subquestion, D_for_subquestion, biq_bin_source_filepath
                )
                ...
                _, vertice_list = max_cut_opt_run_result.split(":")
            ...
            R, _ = primitve_try_qap_solve_with_borders_fixed(
